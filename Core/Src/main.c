...
#define I2S_DMA_BUF_LEN  (I2S_CHUNK_FRAMES * 2U * 2U)
...
int main(void)
{
  ...
  /* Prime H-bridge bootstrap capacitor before opening */
  Motor_Close();
  HAL_Delay(150);
  Motor_Stop();
  HAL_Delay(50);
  ...
  static int8_t   test_buffer[1274];
  ...
  MX_X_CUBE_AI_Process();
    ...
    if (mic_data_ready) {
        ...
        uint8_t pin5 = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_5);
        uint8_t pin6 = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6);
        printf("  [GPIO] PA5(IN1)=%d  PA6(IN2)=%d\r\n", pin5, pin6);
...
static void Motor_Close(void)
{
    ...
    uint8_t r5 = HAL_GPIO_ReadPin(MOTOR_IN1_PORT, MOTOR_IN1_PIN);
    uint8_t r6 = HAL_GPIO_ReadPin(MOTOR_IN2_PORT, MOTOR_IN2_PIN);
    printf("  [MOTOR] CLOSE PA5=%d PA6=%d\r\n", r5, r6);
}
